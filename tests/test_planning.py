...
def mk_item(db, **kw):
    ...
    return services.create(db, Actor(user_id=1), m.Item, **kw)


def test_phase_budget_committed_spent_available(db):
    phase = services.create(db, Actor(user_id=1), m.Phase, name="Phase 1", budget_min=3000, budget_max=5000)
    ...
    services.create(db, Actor(user_id=1), m.Quote, item_id=quoted.id, amount=1500, status="accepted")
...
def test_next_actions_ordering_and_blocked_exclusion(db):
    ...
    services.link_items(db, Actor(user_id=1), u2.id, x.id)
    ...
    services.link_items(db, Actor(user_id=1), blocker.id, blocked.id)
...
def test_needs_quotes(db):
    ...
    for _ in range(3):
        services.create(db, Actor(user_id=1), m.Quote, item_id=quoted_enough.id, amount=100, status="received")
...
def test_savings_projection_months(db):
    services.create(db, Actor(user_id=1), m.Phase, name="Later")
    services.set_setting(db, "monthly_saving_target", "500")
    services.create(db, Actor(user_id=1), m.SavingsEntry, date=date.today().isoformat(), amount=1000)
