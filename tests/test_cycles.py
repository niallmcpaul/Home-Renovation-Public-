...
def make_item(db, title, **kw):
    return services.create(db, Actor(user_id=None), m.Item, title=title, **kw)


def test_self_link_rejected(db):
    a = make_item(db, "A")
    with pytest.raises(ServiceError):
        link_items(db, Actor(user_id=None), a.id, a.id)


def test_direct_cycle_rejected(db):
    a = make_item(db, "A")
    b = make_item(db, "B")
    link_items(db, Actor(user_id=None), a.id, b.id)
    with pytest.raises(ServiceError):
        link_items(db, Actor(user_id=None), b.id, a.id)
...
def test_transitive_cycle_rejected(db):
    ...
    c = make_item(db, "C")
    link_items(db, Actor(user_id=None), a.id, b.id)
    link_items(db, Actor(user_id=None), b.id, c.id)
    with pytest.raises(ServiceError):
        link_items(db, Actor(user_id=None), c.id, a.id)
...
def test_valid_dag_allowed(db):
    ...
    c = make_item(db, "C")
    link_items(db, Actor(user_id=None), a.id, b.id)
    link_items(db, Actor(user_id=None), a.id, c.id)
    link_items(db, Actor(user_id=None), b.id, c.id)
